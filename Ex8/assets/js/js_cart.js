...
function verificarCart(el) {
	var acu = 0;
	for (var i = 0; i < $('.cart-add-item [type="text"]').length; i++) {
		var test = $('.cart-add-item [type="text"]')[i];
		acu += parseInt($(test).val());
	}

	$("#cart-number").text("Cart (" + acu + ") - " + (acu*2) + " €");
...
$min.click(function () { changeValue($(this), -1); });
$max.click(function () { changeValue($(this), 1); });
